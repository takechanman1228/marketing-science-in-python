...
# mailer: "0" = not in the weekly mailer; letter codes (A, C, D, ...) are
# featured placements (A = interior-page feature, the most common). Mirror of
# the display flag — the earlier isin(["A"]) inverted it, counting everything
# that was *not* an A-placement (including "0" = not featured) as featured.
causal["has_mailer"] = (causal["mailer"].astype(str) != "0").astype(int)
...
# Observed weeks BEFORE panel completion: after the reindex below every series
# spans the full week range, so a length filter applied afterward would keep
# everything. Count real observations here so min_weeks actually filters.
observed_weeks = weekly.groupby("unique_id")["WEEK_NO"].nunique()
...
# Filter out series with too few *observed* weeks (counted pre-reindex above)
keep_ids = observed_weeks[observed_weeks >= CONFIG["min_weeks"]].index
